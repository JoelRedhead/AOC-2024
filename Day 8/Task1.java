        ...
        for (Character currentChar : map.keySet()) {
            ...
            for (int i = 1; i < lists.size(); i++) {
                for (int j = 0; j < i; j++) {
                    ...
                    if (lists.get(i).get(0) + iDiff >= 0 && lists.get(i).get(0) + iDiff < newMap.size() && lists.get(i).get(1) + jDiff >= 0 && lists.get(i).get(1) + jDiff < newMap.get(0).size()) {
                        HashSet<Character> newiPos = newMap.get(lists.get(i).get(0) + iDiff).get(lists.get(i).get(1) + jDiff);
                        newiPos.add(currentChar);
                        newMap.get(lists.get(i).get(0) + iDiff).set(lists.get(i).get(1) + jDiff, newiPos);
                    }

                    if (lists.get(j).get(0) - iDiff >= 0 && lists.get(j).get(0) - iDiff < newMap.size() && lists.get(j).get(1) - jDiff >= 0 && lists.get(j).get(1) - jDiff < newMap.get(0).size()) {
                        HashSet<Character> newjPos = newMap.get(lists.get(j).get(0) - iDiff).get(lists.get(j).get(1) - jDiff);
                        newjPos.add(currentChar);
                        newMap.get(lists.get(j).get(0) - iDiff).set(lists.get(j).get(1) - jDiff, newjPos);
                    }
                }
            }
        }
